...
let trx_error ~(loc:Location.t) :
  ('a, Format_doc.formatter, unit, 'b) format4 -> 'a =
  Location.raise_errorf ~loc ~sub:[] ~footnote:(Fun.const None) ?source:None

(* Using the Error in Location
let () =
...
let not_supported loc msg =
  trx_error ~loc "%s is not yet supported within brackets" msg
...
let takedrop : int -> 'a list -> 'a list * 'a list = fun n lst ->
...
let get_level : Parsetree.attributes -> stage = fun attrs ->
  ...
  | Some [{pstr_desc = 
        Pstr_eval ({pexp_desc=Pexp_constant 
                      {pconst_desc=Pconst_integer (ns,None)}},_)}] ->
...
(* Check to see if a path refers to an identifier, exception, or
   ...
   Actually, Ident.global is OK: it returns true for non-local, non-scoped
   identifiers: i.e., global and predefined. This is what we need.
*)
...
(* XXX
   ...
   uid is OK for us if it is either Predef or Item{from=Unit_info.Intf}
   See also Env.get_unit_name () for the current unit. 
   Or, check if the value is in a persistent structure... 
*)
...
let rec is_external = function
  ...
  | Path.Pextra_ty (p,_) -> is_external p
...
let rec path_to_lid : Path.t -> Longident.t = function
  ...
  (* inline record paths cannot be named, so this shouldn't occur *)
 ...
 *)
  | (_,ty_path) when is_external ty_path -> 
...
let qualify_label : Longident.t loc -> label_description -> Longident.t loc =
 fun lid ldesc ->
  ...
  | Tconstr(ty_path, _, _) when is_external ty_path -> 
      Location.mkloc (path_to_lid_but_last ty_path ldesc.lbl_name) loc
  | Tconstr _ ->
      trx_error ~loc
        "Label %s cannot be used within brackets. Put into a separate file."
        ldesc.lbl_name
  | _ ->  trx_error ~loc
          "qualify_label: cannot determine type from label %s in type expr %a"
          ldesc.lbl_name Printtyp.Doc.type_expr ldesc.lbl_res
...
let fixup_type : type_expr -> type_desc -> type_expr = fun te td ->
  Btype.newty3 ~level:(Types.get_level te) ~scope:(get_scope te) td
...
(* Utility to build the typedtree for a one-argument function *)
let texp_fun1 (id:Ident.t) (name:string loc)  (* argument *)
    ~(typ:Types.type_expr) ~(env:Env.t)
    (body:expression) : expression_desc = 
  let pat = {pat_loc = name.loc; pat_extra = [];
             pat_attributes = [];
             pat_type = typ; pat_env = env;
             pat_desc = 
             Tpat_var (id,name, Uid.internal_not_actually_unique)} 
  in
  Texp_function ([{fp_arg_label = Nolabel;
                   fp_param = id;
                   fp_partial = Total;
                   fp_kind = Tparam_pat pat;
                   fp_newtypes = [];
                   fp_loc = name.loc;
                 }],
                 Tfunction_body body)
...
(* Utility to build the typedtree for a one-argument function 
   whose argument is not just a simple variable (it could be a tuple, or,
   often, an array)
*)
let texp_fun1_uncurried 
    (binding_pat:Typedtree.pattern) 
    (template:expression)  (* used as the template for the result: we use
                              the env, location info *)
    (body:expression) : expression = 
  { template with
    exp_desc = 
    Texp_function ([{fp_arg_label = Nolabel;
                     fp_param = Ident.create_local "bngs";
                     fp_partial = Total;
                     fp_kind = Tparam_pat binding_pat;
                     fp_newtypes = [];
                     fp_loc = binding_pat.pat_loc;
                 }],
                 Tfunction_body body);
    exp_type = fixup_type template.exp_type 
...
type !+'a code = {repr:code_repr}[@@unboxed]  (* As suggested by Jeremy Yallop,
                                                 Sep 20, 2023 *)
let code_repr : 'a code -> code_repr = fun x -> x.repr
...
(* Separate from vletbindings those that represent CSPs.
   ...
   The result is a pair: CSP let-bindings and the rest of the bindings
*)
...
(* The most general version with several bindings and several expressions 
   ...
   For the sake of uniformity, accept the empty list of names
 *)
let with_binding_region_gen
  (l:Location.t) (names:string loc list) 
  (f:code_repr array ->                         (* freshly bound variables *)
    (code_repr -> code_repr) ->               (* callback to clean-up code *)
    flvars * 'a) :                            (* a list or other collection *)
  string loc list * flvars * 'a = 
 if names = [] then
   let (vars,es) = f [||] (fun cde -> validate_vars l cde) in
   ([],vars,es)                         (* vars aren't necessarily empty:
                                           may contain virtual bindings *)
 else
...
*)
(* Essentially, this is mapping over the Code *)
let code_wrapper : 
    (Location.t -> Parsetree.expression -> Parsetree.expression) ->
    (Location.t -> code_repr -> code_repr) = fun f l e ->
...
let code_wrapper2 : 
    ...
    (Location.t -> code_repr -> code_repr -> code_repr) = fun f l e1 e2 ->
  let Code (vars1,e1) = validate_vars l e1 in
  let Code (vars2,e2) = validate_vars l e2 in
  Code (merge vars1 vars2, f l e1 e2)
...
let build_sequence : Location.t -> code_repr -> code_repr -> code_repr = 
  code_wrapper2 @@ fun loc e1 e2 -> Ast_helper.Exp.sequence ~loc e1 e2
let build_while : Location.t -> code_repr -> code_repr -> code_repr = 
  code_wrapper2 @@ fun loc e1 e2 -> Ast_helper.Exp.while_ ~loc e1 e2
...
let build_field : Location.t -> code_repr -> Longident.t loc -> code_repr =
 fun loc e lid ->
  code_wrapper (fun loc e -> Ast_helper.Exp.field ~loc e lid) loc e

let build_setfield :
 Location.t -> code_repr -> Longident.t loc -> code_repr -> code_repr =
 fun loc e1 lid e2 ->
  code_wrapper2 (fun loc e1 e2 -> Ast_helper.Exp.setfield ~loc e1 lid e2)
     loc e1 e2
...
let build_send : Location.t -> code_repr -> string -> code_repr =
 fun loc e l ->
  code_wrapper (fun loc e -> Ast_helper.Exp.send ~loc e (mknoloc l)) loc e
...
let build_for 
  (loc:Location.t) (old_name:string loc) (elo:code_repr) (ehi:code_repr)
  (dir:bool) (fbody:code_repr -> code_repr) : code_repr =
...
let lift_as_literal : 
  ...
  match get_desc exp_ty with
    ...
    | Tconstr(p, _, _) when Path.same p Predef.path_int32 ->
        texp_apply (texp_ident "Trx.lift_constant_int32") [exp]
    | Tconstr(p, _, _) when Path.same p Predef.path_int64 ->
        texp_apply (texp_ident "Trx.lift_constant_int64") [exp]
    | Tconstr(p, _, _) when Path.same p Predef.path_nativeint ->
        texp_apply (texp_ident "Trx.lift_constant_nativeint") [exp]
          (* Deal with code type *)
    | _ -> raise CannotLift
...
let lift_constant_int32 : int32 -> code_repr = fun x -> 
  open_code @@ Ast_helper.Exp.constant (Const.int32 x)

let lift_constant_int64 : int64 -> code_repr = fun x -> 
  open_code @@ Ast_helper.Exp.constant (Const.int64 x)

let lift_constant_nativeint : nativeint -> code_repr = fun x -> 
  open_code @@ Ast_helper.Exp.constant (Const.nativeint x)
...
(* Note on effect patterns
   It is such a mess: see that even untypeast.ml got them wrong!
   It itself acknowledges it.
   For the sake of uniformity, we represent exceptional cases
   and patterns as regular exceptional _tupple_ patterns. That is,
       effect p k
   is represented as if it were
       exception (p,k)
   This does not create any confusion as tuples can never be exceptions
   (can never be thrown as exception: everything thrown as an exception
    must have exn type)
   It is only at the very end that we generate the appropriate Ppat_effect
   AST node.
*)
(* convert effect cases to exceptional cases where the exception 
   pattern is a tuple *)
let texp_conv_eff ({c_lhs;c_cont;} as c:value case) : value case = 
  let conv_cont : Ident.t option -> value pattern_desc = function
    | None    -> Tpat_any
    | Some id -> Tpat_var (id,Location.mknoloc (Ident.name id),
                           Uid.internal_not_actually_unique)
  in
  {c with
   c_cont = None;
   c_lhs = {c_lhs with pat_desc = 
            Tpat_tuple [c_lhs;
                        {c_lhs with pat_desc = conv_cont (Option.map fst c_cont)}]}}
...
let rec trx_pattern : 
  ...
  |  { pat_extra=[Tpat_unpack, loc, _attrs]; pat_desc = Tpat_any; _ } ->
        (Ppat_unpack (Location.mkloc None loc),acc)
  ...
  (* we only process value patterns; the latter are computational patterns
  | Tpat_value p -> 
      trx_pattern acc (p :> pattern)
  | Tpat_exception p -> 
      let (p,acc) = trx_pattern acc (p :> pattern) in
      (Ppat_exception p,acc)
  *)
  in
...
(* Patterns are processed left-to-right. The result is the processed
   ...
   Thus the following should hold:
      let (pats,names,_) = trx_pattern_list texp_pats in
      let (pats',acc) =  pattern_subst_list names pats in
      assert (pats = pats');
      assert (acc = [])
   The final result of trx_pattern_list is the pattern binding the names.
  ...
  should have been split up (see Typedtree.split_pattern). The exceptional
  and effectful patterns are actually regular value patterns, only
  mentioned specially in typedtree.
*)
let trx_pattern_list 
    (texp_pats:pattern list) (typ:type_expr) :
     Parsetree.pattern list * string loc list * Typedtree.pattern =
   let (pats, lst) = map_accum trx_pattern [] texp_pats in
   let idnames = List.rev lst in
   let (loc,env) = 
     match texp_pats with p::_ -> (p.pat_loc, p.pat_env) |_ -> assert false in
    (* Pattern representing one binding variable *)
   let var_pat (id,name) =
    {pat_loc = loc; pat_extra = []; pat_env = env;
     pat_desc = Tpat_var (id,name,Uid.internal_not_actually_unique);
     pat_attributes=[];
     pat_type = typ} in
...
let trx_cl : value case list -> type_expr ->
     Parsetree.pattern list * string loc list * Typedtree.pattern
   = fun cl typ -> 
     trx_pattern_list (List.map (fun {c_lhs} -> c_lhs) cl) typ
...
let prepare_cases 
  (loc:Location.t) 
  (evars:flvars)     (* extra free variables used in kontinuation *)
  ...
  *)
  ((pats:Parsetree.pattern list), (old_names:string loc list))
  (* The following function returns the list of pairs of guards and bodies,
     for each clause of the function
   *)
  (fgbodies: code_repr array -> (code_repr option * code_repr) array)
  (* The continuation *)
  (k:Parsetree.case list -> Parsetree.expression) : code_repr =
  let (names,vars,egbodies) = 
         with_binding_region_gen loc old_names @@ fun newvars cleanupf -> 
    let (egbodies,vars) = 
      validate_vars_map loc (fun _loc -> function 
        | (None,cde) -> 
        ...
        | (Some ocde,cde) -> 
            ...
            ((Some oe,e),merge vars ovars))
        (Array.to_list @@ fgbodies newvars)
      in (vars,egbodies)
  in
  let pats = 
...
let build_fun 
  (loc:Location.t)
  ((pats:Parsetree.pattern list), (old_names:string loc list))
  (fgbodies: code_repr array -> 
    code_repr *
    (arg_label * code_repr option * Location.t) array) : code_repr =
  let (names,vars,(body,arginfo)) = 
         with_binding_region_gen loc old_names @@ fun newvars cleanupf -> 
    let (e,arginfo) = fgbodies newvars in
    let Code (varse,e) = cleanupf e in
    let (arginfo,varsa) = 
      validate_vars_map loc 
        (fun _loc -> function
          | (l,None,loc)      -> ((l,None,loc),empty)
          | (l,Some ocde,loc) -> 
              let Code (ovars,oe) = cleanupf ocde in
              ((l,Some oe,loc),ovars))
        (Array.to_list arginfo)
    in
    (merge varsa varse, (e,arginfo))
  in
  let pats = 
      if names = [] then pats else
      let (pats,acc) = pattern_subst_list names pats in
  ...
  let open Parsetree in
    Code(vars,
         Ast_helper.Exp.function_ ~loc
           (List.map2 (fun p (l,eo,loc) -> 
             { pparam_loc = loc;
               pparam_desc = Pparam_val (l,eo,p)})
              pats arginfo)
...
let build_fun_cases
  ...
  ((pats:Parsetree.pattern list), (old_names:string loc list))
  (fgbodies: code_repr array -> 
    (arg_label * code_repr option * Location.t) array *
    Location.t *
    (code_repr option * code_repr) array) : code_repr =
  let (names,vars,(arginfo,cloc,cl)) = 
         with_binding_region_gen loc old_names @@ fun newvars cleanupf -> 
    let (arginfo,cloc,cl) = fgbodies newvars in
    let (arginfo,varsa) = 
      validate_vars_map loc 
        (fun _loc -> function
          | (l,None,loc)      -> ((l,None,loc),empty)
          | (l,Some ocde,loc) -> 
              let Code (ovars,oe) = cleanupf ocde in
              ((l,Some oe,loc),ovars))
        (Array.to_list arginfo) in
    let (cl,varsc) = 
      validate_vars_map loc 
        (fun _loc -> function
          | (None,cde)      -> 
              let Code (vars,cde) = cleanupf cde in
              ((None,cde),vars)
          | (Some ocde,cde) -> 
              let Code (vars,cde) = cleanupf cde in
              let Code (ovars,oe) = cleanupf ocde in
              ((Some oe,cde),merge vars ovars))
        (Array.to_list cl)
    in
    (merge varsa varsc, (arginfo,cloc,cl))
  in
  let pats = 
      if names = [] then pats else
      let (pats,acc) = pattern_subst_list names pats in
      assert (acc = []); pats
  in
  let (pats_args,pats_cl) = takedrop (List.length arginfo) pats in
  let open Parsetree in
    Code(vars,
         Ast_helper.Exp.function_ ~loc
           (List.map2 (fun p (l,eo,loc) -> 
           ...
           None 
           (Pfunction_cases (
            List.map2 (fun pc_lhs (pc_guard,pc_rhs) -> {pc_lhs;pc_guard;pc_rhs})
              pats_cl cl,
            cloc,[])))

(* Build the general let-Parsetree (like the fun-Parsetree) *)
let build_let 
  (loc:Location.t) (recf:bool) 
  ((pats:Parsetree.pattern list), (old_names:string loc list))
  (* The first element of the result in the let body; the others are
     the bound expressions, in order
  *)
  (fgbodies:code_repr array -> code_repr array) :
  code_repr =
  let (names,vars,exps) = 
         with_binding_region_gen loc old_names @@ fun newvars cleanupf -> 
     fgbodies newvars |> Array.to_list |> 
     validate_vars_map loc
      (fun _loc c -> let Code (vars,e) = cleanupf c in (e,vars)) |>
     fun (es,vars) -> (vars,es)
  in
  let pats = 
      if names = [] then pats else
  ...
  | ebody :: ebounds ->
    Code(vars,
      Ast_helper.Exp.let_ ~loc (if recf then Recursive else Nonrecursive)
          (List.map2 (fun lhs rhs -> Ast_helper.Vb.mk ~loc:lhs.ppat_loc lhs rhs)
           pats ebounds) 
         ebody)
...
let build_match 
  (loc:Location.t)
  (* (binding) patterns and the list of variables they bind *)
  (pon: Parsetree.pattern list * string loc list)
  (ec:code_repr)                        (* scrutinee *)
  (nregular:int)                        (* the first nregular cases
                                           produced by fgbodies below
                                           are regular; the rest are exceptional
                                           and effectful
                                        *)
  (fgbodies: code_repr array -> (code_repr option * code_repr) array) : 
  code_repr =
  ...
  prepare_cases loc evars pon fgbodies @@ fun cases ->
    Ast_helper.Exp.match_ ~loc exp begin
      let (rc,ec) = takedrop nregular cases in
       rc @ List.map (function 
        | {pc_lhs = {ppat_desc = Ppat_tuple[p1;p2]}} as c -> 
            {c with pc_lhs = {c.pc_lhs with ppat_desc = Ppat_effect (p1,p2)}}
        | {pc_lhs} as c -> 
            {c with pc_lhs = {pc_lhs with ppat_desc = Ppat_exception pc_lhs}}
                     ) ec
    end
...
let rec trx_translate : expression -> expression = fun exp ->
  ...
  *)
  ...
  (* About binding patterns: see general let, function, match and try
     Convert a list of patterns and some expression 
     (of the type t, where t is code_repr or code_repr array,
      code_repr * code_repr, etc.) that contains
     variables bound by that pattern into a present-stage function
       code_repr array -> t
     The argument is the new names for the variables bound in patterns.
     If all the patterns are simple variable patterns, then this function
     converts a future-stage function fun (x1:t1) ... (xn:tn) -> (e:t)
     into a present-stage function 
     fun (x1:t1 code) ... (xn:tn code) -> (e:t code)
     The function is actually uncurried, and we use array instead of a tuple.
     Using array forces a single type to all arguments. Although
...
and trx_translate_ : expression -> expression = fun exp ->
  ...
  | Texp_typed_hole -> 
      texp_code ~node_id:"*typed_hole*" exp.exp_loc (Pexp_ident (Location.mkloc (Longident.Lident "_") exp.exp_loc))
  | Texp_ident (p,li,vd)  ->
  ...
  | Texp_constant cst -> 
     ...
     (* The most common case of let-expressions: non-recursive
        let x = e in body *)
  | Texp_let (Nonrecursive,[{vb_pat = {pat_desc = Tpat_var (id,name,_)} as pat;
                     vb_expr = e}],ebody) ->
      let pat_type = mk_type_code pat.pat_type in
      texp_apply (texp_ident "Trx.build_let_simple_nonrec") 
        [texp_loc exp.exp_loc;
         texp_string_loc name;
         trx_translate e;
         { exp with
           exp_desc = texp_fun1 id name ~typ:pat_type ~env:pat.pat_env
             (trx_translate ebody);
           exp_type = fixup_type exp.exp_type
               (Tarrow (Nolabel,pat_type, 
  ...
  | Texp_let (recf,vbl,ebody) ->
     (*   No longer needed: Jeremy Yallop integrated the test into the typecore
      let check_letrec ({vb_pat=p;vb_expr=e} as vb) =
        ...
        | Tpat_var (_,_) -> ()
        | _ -> 
            trx_error ~loc:p.pat_loc
            "Only variables are allowed as left-hand side of `let rec'"
        end;
        match e.exp_desc with
        | Texp_function _ -> ()
        | _ -> 
            trx_error ~loc:vb.vb_loc
            "Recursive let binding must be to a function"
            (* Location.print e.exp_loc *)
      in if recf = Recursive then
        List.iter check_letrec vbl;
      *)
      (* Pick up all patterns and process them, extracting all bound variables
         and extending their scope to the whole expression.
      ...
      *)
      let (pl,names,binding_pat) = 
        trx_pattern_list (List.map (fun {vb_pat} -> vb_pat) vbl) 
          (mk_type_code (Btype.newgenvar ()))
      in
      texp_apply (texp_ident "Trx.build_let") 
        [texp_loc exp.exp_loc;
         texp_bool (recf = Recursive);
         texp_pats_names pl names;
         texp_fun1_uncurried binding_pat exp @@
             texp_array 
               (trx_translate ebody ::
                   List.map (fun {vb_expr} -> trx_translate vb_expr) vbl)
       ]

     (* General functions
        Again, since the scope of all bound variables has been worked out
        and they are all uniquely renamed (keeping timestamp in mind),
        we can assume they are all bound at the same scope and play the same
        trick we did with let.
        That is, we represent the function

           Texp_function ([p1;...;pn], Tfunction_cases[(pc1,e1);(pc2;e2)])
         that is, 
           fun p1...pn -> function pc1 -> e1 | pc2 -> e2
       by the list of cases
        [(p1,edummy);...;(pn,edummy);(pc1,e1);(pc2;e2)]

       and the function
           Texp_function ([p1;...;pn], Tfunction_body e)
         that is, 
           fun p1...pn -> e
       by the list of cases
        [(p1,edummy);...;(pn-1,edummy);(pn,e1)]

      To distinguish the dummy cases from real ones, we also pass the integer n
      for the number of actual parameters. In the Tfunction_cases case,
      it is the number of parameters: non-negative number.
      In the case of Tfunction_body, it is the number of parameters, negated
      (it is hence always negative).

  *)
  | Texp_function (paramlst, Tfunction_body e) -> 
      let ps = paramlst |> List.map (function
          | {fp_newtypes=nt::_} -> 
              trx_error ~loc:nt.loc
              "new type declarations such as %s are not supported" nt.txt
          | {fp_arg_label;fp_kind=Tparam_pat p;fp_loc} -> 
              (fp_arg_label,None,p,fp_loc)
          | {fp_arg_label;fp_kind=Tparam_optional_default (p,e);fp_loc} -> 
              (fp_arg_label,Some e,p,fp_loc))
      in
      let (pl, names, binding_pat) =
        trx_pattern_list (List.map (fun (_,_,p,_) -> p) ps)
                         (mk_type_code (Btype.newgenvar ()))
      in
      texp_apply (texp_ident "Trx.build_fun") 
        [texp_loc exp.exp_loc;
         ...
         texp_fun1_uncurried binding_pat exp @@
           texp_tuple [
             trx_translate e;
             texp_array (List.map (fun (l,eo,_,loc) -> 
                    texp_tuple [texp_label l;
                                texp_option @@ Option.map trx_translate eo;
                                texp_loc loc])
                         ps)
                      ]
       ]

  | Texp_function (paramlst, Tfunction_cases {cases;loc}) ->
      let ps = paramlst |> List.map (function
          | {fp_newtypes=nt::_} -> 
              trx_error ~loc:nt.loc
              "new type declarations such as %s are not supported" nt.txt
      ...
      let (pl, names, binding_pat) =
        trx_pattern_list
          (List.map (fun (_,_,p,_) -> p) ps @
           List.map (fun {c_lhs} -> c_lhs) cases)
          (mk_type_code (Btype.newgenvar ()))
      in
      texp_apply (texp_ident "Trx.build_fun_cases") 
        [texp_loc exp.exp_loc;
         texp_pats_names pl names;
         texp_fun1_uncurried binding_pat exp @@
           texp_tuple [
            ...
            texp_array (List.map (fun {c_guard;c_rhs;_} -> 
              texp_tuple [texp_option @@ Option.map trx_translate c_guard;
                          trx_translate c_rhs])
                          cases)
                      ]
       ]
  ...
  | Texp_match (e,cl',effcl,_) ->
      (* split clauses cl' into lists of ordinary and exceptional cases *)
      let rec collect_cases ((rcl:value case list), (ecl:value case list)) : 
        computation case list -> value case list * value case list = function
      ...
      (* handle all cases uniformly, including effect *)
      let cl = rcl @ ecl @ List.map texp_conv_eff effcl in
      let (pl,names,binding_pat) = 
  ...
  | Texp_try (e,cl,effcl) ->                 (* same as Texp_match *)
      (* convert effect cases to exceptional cases where the exception
         pattern is a tuple *)
      let cl = cl @ List.map texp_conv_eff effcl in
      let (pl,names,binding_pat) = 
        trx_cl cl (mk_type_code (Btype.newgenvar ())) in
  ...
  | Texp_record {fields;representation=_r;extended_expression} ->
      texp_apply (texp_ident "Trx.build_record")
        [texp_loc exp.exp_loc; 
         ...
         texp_option (Option.map trx_translate extended_expression)]
  ...
  | Texp_ifthenelse (e,et,efo) ->
      texp_apply (texp_ident "Trx.build_ifthenelse")
        [texp_loc exp.exp_loc; 
         trx_translate e;
         trx_translate et;
	 texp_option (Option.map trx_translate efo)]
  ...
  | Texp_for (id, pat, elo, ehi, dir, ebody) ->
      ...
      texp_apply (texp_ident "Trx.build_for") 
        [texp_loc exp.exp_loc;
         ...
         { exp with
           exp_desc = texp_fun1 id name ~typ:var_typ ~env:exp.exp_env
             (trx_translate ebody);
           exp_type = fixup_type exp.exp_type
...
let make_match : code_repr -> code_repr list -> code_repr =
  ...
  let case_fn fn acc = 
   let open Parsetree in match fn.pexp_desc with
    | Pexp_function ([],_,Pfunction_cases (caselist,_,_)) -> caselist @ acc
    | Pexp_function ([{pparam_desc=Pparam_val (Nolabel,None,p)}],_,
                     Pfunction_body e) -> 
        {pc_lhs=p;pc_guard=None;pc_rhs=e}::acc
  ...
  let iter = 
     {defl with expr =
       ...
       fun this exp ->
        ...
        | Pexp_unreachable 
          -> ()
        | Pexp_constant {pconst_desc = Pconst_string _}
